...
describe('account', function() {
    ...
    it('POST /privkey', function(done) {
        const tvs = JSON.parse(fs.readFileSync("test/test-vectors/1.test-keys-private-generated.json"));
        const req = tvs.map(tv => {
            return request(app)
                .post('/ajax/privkey')
    ...
    it('POST /privkey/sign', function(done) {
        const tvs = JSON.parse(fs.readFileSync("test/test-vectors/2.test-sign-private.json"));
    ...
    it('GET /pubkey/verify', function(done) {
        const tvs = JSON.parse(fs.readFileSync("test/test-vectors/2.test-sign-private.json"));
        const req = tvs.slice(0,100).map(tv => {
            return request(app)
